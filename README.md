# 2026年07月25日07时50分 免费节点订阅说明

本目录中的配置整理自公开节点页面。

文件已于 2026年07月25日07时50分 下载、合并和校验。它们是当天的静态快照，不会自动更新。
...
本仓库提供手动触发的 GitHub Actions 工作流。运行时输入日期后，会自动下载、合并、清理、校验并上传生成结果。若启用发布选项，工作流只会把生成产物同步到指定产物仓库，不会把生成脚本和自动化流程写入产物仓库。

## 发布检查

- 不要提交本机客户端目录、运行日志、缓存文件或个人配置文件。
- 不要提交 `.env`、API Key、访问令牌、私有订阅链接或账号凭据。
- 推送公开仓库前，请确认你愿意公开这些节点配置；公开后可能被搜索引擎和第三方缓存收录。
