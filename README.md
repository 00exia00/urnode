# 2026年09月05日04时16分 免费节点订阅说明

本目录中的配置整理自公开节点页面。

文件已于 2026年09月05日04时16分 下载、合并和校验。它们是当天的静态快照，不会自动更新。
...
### `clash-merged-20260905.yaml`

- 适用于使用 Clash Meta（Mihomo）内核的客户端，例如 Clash Verge Rev。
- 尝试合并页面上的 Clash 订阅链接 0-4；无法下载或解析的附加源会被跳过并记录在文件头部。
- 可用性测试后保留 3002 条节点。
...
节点已于 2026-09-05 12:16 CST 通过对应代理内核实际访问 `https://cp.cloudflare.com/generate_204`。只有至少一次测试成功的节点才会进入发布文件；测试失败、超时、格式不受支持及指向非公网地址的节点均已移除。

- Clash：原始 4418，可测试 4410，通过 3002
- V2Ray：原始 6772，可转换测试 6584，通过 4121
- Sing-box：原始 15，可测试 14，通过 11

测试结果只代表生成时从 GitHub Actions 运行环境访问成功，免费节点仍可能随后失效、限速或因使用者所在网络不同而无法连接。
