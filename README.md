# 2026年09月04日18时59分 免费节点订阅说明

本目录中的配置整理自公开节点页面。

文件已于 2026年09月04日18时59分 下载、合并和校验。它们是当天的静态快照，不会自动更新。
...
- 可用性测试后配置包含 12 个代理节点（另保留必要的策略出站）。
- JSON 语法和主要结构已校验。
...
节点已于 2026-09-05 02:59 CST 通过对应代理内核实际访问 `https://cp.cloudflare.com/generate_204`。只有至少一次测试成功的节点才会进入发布文件；测试失败、超时、格式不受支持及指向非公网地址的节点均已移除。

- Clash：原始 4418，可测试 4412，通过 1675
- V2Ray：原始 6772，可转换测试 6586，通过 2777
- Sing-box：原始 15，可测试 14，通过 12
